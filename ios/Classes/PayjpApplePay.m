...
- (NSArray<NSString *> *)supportedEvents {
    return @[@"onApplePayProducedToken",
...
RCT_EXPORT_METHOD(makeApplePayToken:(NSDictionary *)arguments
                  resolve:(RCTPromiseResolveBlock)resolve
                  reject:(__unused RCTPromiseRejectBlock)reject) {
    NSAssert([arguments[@"appleMerchantId"] isKindOfClass: [NSString class]], @"invalid appleMerchantId");
    ...
    NSDecimalNumber *amount = [NSDecimalNumber decimalNumberWithString:arguments[@"summaryItemAmount"]];
    PKPaymentSummaryItem *summaryItem =
    [PKPaymentSummaryItem summaryItemWithLabel:arguments[@"summaryItemLabel"]
                                        amount:amount];
...
RCT_EXPORT_METHOD(completeApplePay:(BOOL)isSuccess
        ...
        if (@available(iOS 11.0, *)) {
            ...
            if (isSuccess) {
                ...
                NSError *error = [NSError errorWithDomain:RNPAYErrorDomain
                                                 ...
                                                 userInfo:userInfo];
                completion([[PKPaymentAuthorizationResult alloc]
                            initWithStatus:PKPaymentAuthorizationStatusFailure errors:@[error]]);
            }
        } else {
...
- (void)createTokenWithPaymentToken:(PKPaymentToken *)paymentToken {
    __weak typeof(self) wself = self;
    [PAYAPIClient.sharedClient createTokenWith:paymentToken
...
- (void)paymentAuthorizationViewController:(PKPaymentAuthorizationViewController *)controller
                       didAuthorizePayment:(PKPayment *)payment
                                   handler:(void (^)(PKPaymentAuthorizationResult * __nonnull))completion API_AVAILABLE(ios(11.0)) {
    self.completionHandler = completion;
    [self createTokenWithPaymentToken:payment.token];
}
